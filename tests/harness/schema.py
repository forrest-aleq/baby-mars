...
class ValidationRule(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Validation type: work_unit_count, supervision_mode, no_ethical_flags, escalation, capability_used",
    )
    ...
    capability: Optional[str] = Field(
        default=None, description="Capability key for capability_used validation"
    )
...
class ExpectedOutput(BaseModel):
    ...
    work_units: list[ExpectedWorkUnit] = Field(
        default_factory=list, description="Expected work units to be generated"
    )
    capabilities_used: list[str] = Field(
        default_factory=list, description="Expected Stargate capability keys"
    )
    should_escalate: bool = Field(
        default=False, description="Whether the agent should escalate to human"
    )
    should_flag_exceptions: bool = Field(
        default=False, description="Whether the agent should flag exceptions"
    )
    ethical_concerns: bool = Field(
        default=False, description="Whether the request has ethical concerns"
    )
...
class TestCase(BaseModel):
    ...
    tags: list[str] = Field(
        default_factory=list, description="Tags for filtering (e.g., 'critical', 'edge_case')"
    )
...
class PersonaMetadata(BaseModel):
    ...
    def model_post_init(self, __context):
        ...
        object.__setattr__(
            self, "automation_susceptibility", max(1, min(5, self.automation_susceptibility))
        )
