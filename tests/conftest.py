...
def sample_state_with_message(sample_state):
    ...
    sample_state["messages"] = [
        {
            "role": "user",
            "content": "Process invoice #1234 for $5,000 from Acme Corp"
        }
    ]
    return sample_state
...
def sample_note():
    ...
    from src.state.schema import Note
    return {
        "note_id": "note-001",
...
def hierarchical_belief_graph():
    ...
    foundation = create_belief(
        statement="Financial accuracy is paramount",
        category="moral",
        initial_strength=0.95
    )
    ...
    derived2 = create_belief(
        statement="Journal entries require double-entry",
        category="technical",
