...
class BeliefGraph:
    ...
    def update_belief_from_outcome(
        ...
        new_strength = max(0.0, min(1.0,
            old_strength + LEARNING_RATE * total_signal
        ))
    ...
    def check_invalidation_allowed(
        ...
        threshold = belief.get(
            ...
            INVALIDATION_THRESHOLDS[belief["category"]]
        )
        ...
        if current >= threshold:
            return (False, f"Belief strength {current:.2f} exceeds invalidation threshold {threshold:.2f} for {belief['category']} category. Human confirmation required.")
    ...
    def serialize(self) -> str:
        ...
        return json.dumps({
            ...
            "edges": [
                (u, v, dict(d)) for u, v, d in self.G.edges(data=True)
            ],
